...
# 4）你问的重点：**用 模板 + SFINAE 在编译期判断类型是否支持指定方法签名**
...
```cpp
#ifndef MATRIX_STATIC_ASSERTS_HPP_
#define MATRIX_STATIC_ASSERTS_HPP_

#define STATIC_ASSERT_ARITHMETIC(T) \
    static_assert(std::is_arithmetic_v<T>, "Can only be used with arithmetic types.")

#endif //MATRIX_STATIC_ASSERTS_HPP_
```

这是项目里给**模板类型参数 `T` 做编译期约束**的小工具头文件，用在 `maths.hpp` 的泛型数学函数上。

---

## 这个文件是什么

```1:7:core/module/common/include/common/static_asserts.hpp
#ifndef MATRIX_STATIC_ASSERTS_HPP_
#define MATRIX_STATIC_ASSERTS_HPP_
...
| ----------------------------- | ----------------------------- |
| 头文件保护宏                        | 防止重复 `#include`               |
| `STATIC_ASSERT_ARITHMETIC(T)` | 宏，展开成一条 `static_assert`       |
| `std::is_arithmetic_v<T>`     | C++17 类型特征：判断 `T` 是否为**算术类型** |

**算术类型**包括：`int`、`float`、`double`、`bool`、`char` 及其 `signed/unsigned`、各宽度整型等——能参与 `+ - * /`、能和 `0` 比较的类型。

---

## 为什么对抽象 `T` 要这样处理

`maths.hpp` 里有多处**模板函数**，`T` 在写代码时是抽象的：

```40:56:core/module/common/include/common/maths.hpp
    template<typename T>
    inline auto abs(T v){
    ...
    template<typename T>
    inline auto sqrt(T v){ 
        STATIC_ASSERT_ARITHMETIC(T);
        return std::sqrt(v); 
    }

    template<typename T>
    inline auto square(T v){ 
        STATIC_ASSERT_ARITHMETIC(T);
        return v * v; 
    }
```
...
这些函数语义上只适用于**数值类型**。如果不对 `T` 加限制，调用方可能写出：

```cpp
maths::abs(std::string("hello"));  // 没有约束时可能拖到实例化/链接才出问题
maths::square(someCustomClass);      // v * v 可能根本编译不过，报错还难懂
```

在模板里加 `STATIC_ASSERT_ARITHMETIC(T)` 的目的：

### 1. 编译期拦截，而不是运行期才暴雷

`static_assert` 在**编译阶段**检查。一旦 `T` 不是算术类型，直接编译失败，并带上明确信息：

```
error: static assertion failed: Can only be used with arithmetic types.
```

比模板实例化后一堆晦涩的 `no match for operator*` 更好排查。

### 2. 把「契约」写在函数入口

对 C++ 开发者来说，这类似：

- 运行时：前置条件 `assert` / `throw`
- 编译时：`static_assert` / `concept`

这里选的是：**这个泛型函数只接受算术类型**，在最早、最便宜的地方（编译器） enforce。

### 3. 用宏集中复用，避免复制粘贴

`abs`、`sqrt`、`square`、`sign` 都要同一规则。抽成宏：

- 改一处，处处生效
- 报错文案统一
- 以后还能加 `STATIC_ASSERT_FLOATING(T)` 等同类宏

### 4. 比 `concept` 更轻、兼容面更广
...
但项目里用 `std::is_arithmetic_v` + `static_assert`：

- 不依赖 C++20
- 能覆盖 `int` 等整数（`sign`、`square` 对整数也合理）
- 和现有 `maths` 工具风格一致
...
```mermaid
flowchart TD
    A["调用 maths::abs(v)"] --> B["编译器实例化 template<typename T> abs(T)"]
    B --> C{"std::is_arithmetic_v<T> ?"}
    C -->|是| D["编译通过，生成代码"]
    C -->|否| E["static_assert 失败，编译中止"]
```

**没有运行期开销**：断言不参与执行，只影响能否编译通过。

---

## 和「抽象 `T`」的关系（一句话）

模板里的 `T` 太宽，编译器默认**什么类型都能传**。  
`STATIC_ASSERT_ARITHMETIC(T)` 是在说：**这些数学函数只给数字用**；传错类型时，在编译期就拒绝，并给出清晰错误，而不是让奇怪的类型混进 `std::abs` 或 `v * v`。
...
| --------------------------------------- | ---------------- | ------------------- |
| 不约束 `T`                                 | 写法最简             | 误用时报错难懂             |
| `static_assert` + `is_arithmetic_v`（当前） | 编译期、零开销、C++17 即可 | 错误信息不如 concept 直观   |
| C++20 `concept`                         | 语义最清晰            | 需 C++20，且要区分整数/浮点策略 |

---

## 总结

`static_asserts.hpp` 不是业务逻辑，而是**泛型数学函数的编译期类型护栏**。  
对抽象 `T` 这样处理，是因为 `maths::abs/sqrt/square/sign` 在语义上只适用于算术类型；用 `static_assert` 在模板实例化时立刻失败，既安全又无运行期成本，还通过宏避免在多个模板里重复写同一条约束。
