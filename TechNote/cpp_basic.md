...
`explicit` 关键字用于修饰构造函数，防止编译器做“隐式类型转换”。

## 含义

当构造函数加了 `explicit`：

- `Route r = 1.2;` 这种隐式转换就会被禁止
- 只能写 `Route r(1.2);` 或 `Route r{1.2};`

如果没有 `explicit`，编译器会允许把 `double` 自动转换成 `Route`，这可能导致意外的类型转换和错误。

## 用法

```cpp
struct Route {
    explicit Route(double cost) : cost(cost) {}
};
```

这个写法表示：

- 只有显式调用构造函数，才能创建 `Route`
- 不能通过 `double` 自动生成 `Route`
...
`explicit` 就是“显式”的意思。它告诉编译器：这个构造函数只能显式调用，不能隐式转换。对于只有一个参数的构造函数，推荐添加 `explicit`，避免不必要的自动类型转换。
